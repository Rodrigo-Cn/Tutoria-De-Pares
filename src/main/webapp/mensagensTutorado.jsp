...
<body>
    ...
    <div id="navbar">
        <div id="navbar-logo">
            <div> <a href="voltarParaMainTutorado?id=<%= request.getParameter("id")%>"> <img src="imagens/mensagens/Frame 1.png" id="navbar-image" class="tracking-in-expand-forward-top" alt="frame.png"></div> </a>
        ...
        <div id="navbar-inicio">
            <a href="voltarParaMainTutorado?id=<%= request.getParameter("id")%>" style="margin-top: 3.4%;"><img id="navbar-image2" class="tracking-in-expand-forward-top"
    ...
    <div id="section">
        ...
        <div id="section-card">
            ...
            <%
                int idTutorado = ((Tutorado) request.getAttribute("tutorado")).getId();
            %>
                ...
                <% for(int i=0; i<((Meta) request.getAttribute("meta")).getMensagens().size(); i++) { %>
                    <div class="card">
                         <div class="title-card">
                              ...
                              <div class="title-card-options">
                                    <% if( idTutorado==((Meta) request.getAttribute("meta")).getMensagens().get(i).getUsuario().getId() ) { %>
                                        <a href="selecionaMensagemTutorado?codigoMeta=<%=((Meta) request.getAttribute("meta")).getCodigo()%>&codigoTutoria=<%=((Tutoria) request.getAttribute("tutoria")).getCodigo()%>&id=<%= ((Tutorado) request.getAttribute("tutorado")).getId() %>&codigoMensagem=<%=((Meta) request.getAttribute("meta")).getMensagens().get(i).getCodigoMensagem()%>"><img id="iniciar-config" class="iniciar-config" src="imagens/mensagens/config.png" alt=""></a>
                                        <a onclick="confirmar(<%= ((Meta) request.getAttribute("meta")).getCodigo() %>, <%=((Tutoria) request.getAttribute("tutoria")).getCodigo()%>, <%=((Tutorado) request.getAttribute("tutorado")).getId()%>, <%= ((Meta) request.getAttribute("meta")).getMensagens().get(i).getCodigoMensagem()%>)"><img src="imagens/mensagens/icons8-lixo-50.png" alt=""></a>
                                    <% } %>
                              </div>
                         </div>
                         <div class="body-card">
        ...
        <form id="form-enviar-mgs" action="enviarMensagemTutorado">
    ...
    <script src="scripts/mensagens/apagarMensagemTutorado.js"></script>
</body>
</html>